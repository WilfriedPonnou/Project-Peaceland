...
object Reports{ 
  ...
  def stringToReport(s: String): Reports = {
    ...
    val id=reportAttributes(0).toInt
    val lat=reportAttributes(1)
    val lon=reportAttributes(2)
    val citizens=reportAttributes(3).split(",").map(citizenData=>Citizen.stringToCitizen(citizenData))
    val words=reportAttributes(4).split(",")
    val date=reportAttributes(5)
